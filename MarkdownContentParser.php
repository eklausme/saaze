...
/* Extension modeled after: https://saaze.dev/docs/extending
   ...
   Elmar Klausmeier, 09-May-2023, added hackLNHighlight() to accomodate for PrismJS line highlighting
*/
...
//printf("toHtml(): fileToRender=%s, last=%d, start=%d, end=%d, %s[%s]\n",$GLOBALS['fileToRender'],$last,$start,$end,substr($content,$start,$end-$start+2),substr($content,0,12));
	...
	/**
	 * Dirty hack to enable linenumber highlighting with PrismJS
	 * It operates on the HTML generated by the Markdown to HTML converter, here MD4C
	 * Replace </p>\n<pre><code class="language-XXX ABC" with </p>\n<pre ABC><code class="language-XXX">
	 * Below regex does not work as we need to decode the HTML entities introduced by MD4C
		$html = preg_replace(
			"/<\/p>\n<pre><code class=\"language-(\w+)\[([ =;\&\w\"\-]+)\]\">/",
			"</p>\n<pre $2><code class=\"language-$1\">",
			$html
		);
	 */
	...
	public function toHtml(string $content, Entry &$entry) : string {
		...
		$html = isset(\Saaze\Config::$H['global_ffi']) ?
			\FFI::string( \Saaze\Config::$H['global_ffi']->md4c_toHtml($modContent) )	// Markdown to HTML conversion
			: md4c_toHtml($modContent);	// Markdown to HTML conversion with MD4C PHP extension
		...
		* * */
		if ($entry->data) {
			...
			if ($entry->data['prismjs'] ?? false) {
				$html = $this->hackLNHighlight($html);
			}
		}
